...
/**
   ...
   date 2014-5-3
 */
class DiscountOrderAction extends BackendAction{
    public function _initialize() {
    parent::_initialize();
  }
  public function index(){
     $model = M('discount_order o');
    //取得满足条件的记录数
      ...
      $voList = $model->join('left join wx_member m on m.id=o.member_id')
                      ->join('left join wx_discount d on d.id=o.discount_id')
                      ->field('d.discount,d.title,m.wxname,m.mobile,d.end_time,o.*')->limit($p->firstRow . ',' . $p->listRows)->select ();
         ...
         if ($result['status']==1){
          ...
          $row = $model->where(array('id'=>$id))->save(array('status'=>1,'use_time'=>$time)); 
            if($row>0){
              $data['s'] = 0;
              $data['msg'] = '兑换成功';
              $data['span'] = '<span style="color:red;">已使用</span>';

...
<body>
  <div class="app-wrap container">
    <form class="head-search mb5" method="get" action="<?php echo U('Wap/Booking/index');?>">
    ...
    <ul class="ticket-list">
       <?php if(is_array($list)): foreach($list as $key=>$list): ?><li>
        <a href="<?php echo U('Wap/Booking/details',array('id'=>$list['id']));?>" <?php if($list['num'] == $list['pay_num']): ?>class="js_not_ticket"<?php endif; ?>> <!-- 若没票，请标识class为:js_not_ticket -->
          <div class="ticket-item">
            <div class="schedule-tiem"><i class="fa fa-clock-o"></i><?php echo (date('m月d日',$list['start_time'])); ?>-<?php echo (date('m月d日',$list['end_time'])); ?></div>
            <div class="schedule-tit">《<?php echo $list['title'];?>》</div>
          </div>
          <button class="btn btn-sm btn-warning"><i class="fa fa-cart-plus"></i>订票</button>
        </a>

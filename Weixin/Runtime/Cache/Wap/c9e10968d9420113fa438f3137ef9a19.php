  ...
  <div class="app-wrap container">
        ...
        <div class="swiper-wrapper">
           <?php $adv = explode(',',$data['adv']); ?>
            <?php if(!empty($data['adv'])){ ?>
             <?php if(is_array($adv)): foreach($adv as $key=>$advlist): ?><div class="swiper-slide"><a href="#"><img src="<?php echo $advlist;?>"></a></div><?php endforeach; endif; ?>
             <?php } ?>
        </div>
    ...
    <div class="video-list clearfix">
      <?php if(is_array($list)): foreach($list as $key=>$list): ?><div class="video-item">
        <a href="<?php echo U('Wap/Artice/video',array('id'=>$list['id']));?>">
          <img src="<?php echo $list['thumb'];?>" />
          <div class="title">《<?php echo str_cut($list['title'],15);?>》</div>
        </a>

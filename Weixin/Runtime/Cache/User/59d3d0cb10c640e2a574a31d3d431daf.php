...
<div class="main span10">
    <h4>编辑菜单<a href="<?php echo U(GROUP_NAME.'/Menu/index');?>" class="pull-right" ><i class="icon-backward"></i> <strong>返回列表</strong></a></h4>
<hr>
    <form action="<?php echo U(GROUP_NAME.'/Menu/update');?>" method="post" accept-charset="utf-8" class="well form-inline" id="f" onSubmit="return qjy_ajaxform(this,function(d){opt_ok_return(d)})"> 
      <table class="table table-bordered table-wordpress postlist-table">
        ...
        <tr>
          ...
          <td>
             <select name="pid">
                ...
                <?php if(is_array($cate)): foreach($cate as $key=>$list): ?><option value="<?php echo $list['id'];?>" <?php if($list['id'] == $vo['pid']): ?>selected="selected"<?php endif; ?>><?php echo $list['title'];?></option><?php endforeach; endif; ?>
             </select>
          </td>
        </tr>   
        <tr>
        ...
        <tr>
          ...
          <td><input type="text" name="url" value="<?php echo $vo['url'];?>" id="url" class="span8"/></td>
        </tr> 

...
<a href="<?php echo U(GROUP_NAME.'/Schedule/add');?>" class="btn btn-success"><i class="icon-plus icon-white"></i> 添加</a>
</div>
<h4>演出排期</h4>
<hr/>
  <table class="table table-bordered table-wordpress postlist-table">
    ...
    <tbody>
        ...
        <td><?php echo $list['playtime'];?></td>
        <td><?php echo $list['address'];?></td>
        <td><?php echo $list['num'];?>张</td>
         <td><?php echo $list['pay_num'];?>张</td>
        <td><?php echo ($list['num']-$list['pay_num']);?>张</td>
          <td><?php echo (date('Y-m-d H:i',$list['start_time'])); ?>至<br/><?php echo (date('Y-m-d H:i',$list['end_time'])); ?></td>      
        <td>
          <?php if($list['status'] == 0): ?>正常<?php else: ?>状态<?php endif; ?>
        </td>
        <td>
          <div class="btn-toolbar">
            <div class="btn-group">
              <a href="<?php echo U(GROUP_NAME.'/Schedule/edit',array('id'=>$list['id']));?>" class="btn btn-mini" ><i class="icon-edit"></i> 更改</a>

...
<div class="main span10">
    <h4>添加内容<a href="<?php echo U(GROUP_NAME.'/Video/index');?>" class="pull-right" ><i class="icon-backward"></i> <strong>返回列表</strong></a></h4>
<hr>
    <form action="<?php echo U(GROUP_NAME.'/Video/insert');?>" method="post" accept-charset="utf-8" class="well form-inline" id="f" onSubmit="return qjy_ajaxform(this,function(d){opt_ok_return(d)})"> 
      <table class="table table-bordered table-wordpress postlist-table">
        <tbody>
          <tr>
            ...
            <td> <select name="cate_id">
                <?php if(is_array($cate)): foreach($cate as $key=>$list): ?><option value="<?php echo $list['id'];?>"><?php echo $list['subject'];?></option><?php endforeach; endif; ?>
             </select>
        ...
        <tr>
          ...
          <td>
            ...
            <input  class="btn" id="uploadfile" value="选择文件"><span>上传文件大小<?php echo ini_get('upload_max_filesize');?></span>
          </td>
        </tr> 
        <tr>
